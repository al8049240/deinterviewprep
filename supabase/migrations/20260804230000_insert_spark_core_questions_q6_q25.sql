...
-- Dynamically resolves Apache Spark topic ID and Spark Core subtopic ID
-- Inserts into de_mobile_app."quiz-question" and de_mobile_app."quiz-answer"
...
BEGIN
  ...
  SELECT id::INTEGER INTO v_topic_id
  FROM de_mobile_app.topics
  WHERE name = 'Apache Spark'
  LIMIT 1;

  IF v_topic_id IS NULL THEN
    RAISE NOTICE 'Apache Spark topic not found. Aborting.';
    RETURN;
  END IF;

  -- Resolve Spark Core subtopic ID
  SELECT id::INTEGER INTO v_subtopic_id
  FROM de_mobile_app.subtopics
  WHERE topic_id = v_topic_id
    AND (name ILIKE '%spark core%' OR name ILIKE '%core%')
  ORDER BY
    CASE WHEN name ILIKE '%spark core%' THEN 0 ELSE 1 END
  LIMIT 1;

  IF v_subtopic_id IS NULL THEN
    RAISE NOTICE 'Spark Core subtopic not found. Aborting.';
    RETURN;
  END IF;

  RAISE NOTICE 'Using topic_id=%, subtopic_id=%', v_topic_id, v_subtopic_id;
  ...
  VALUES
    ('spark_q6',  v_topic_id, v_subtopic_id, 'mcq',  'junior',
    ...
    ('spark_q7',  v_topic_id, v_subtopic_id, 'mcq',  'middle',
    ...
    ('spark_q8',  v_topic_id, v_subtopic_id, 'mcq',  'senior',
    ...
    ('spark_q9',  v_topic_id, v_subtopic_id, 'mcma', 'junior',
    ...
    ('spark_q10', v_topic_id, v_subtopic_id, 'mcq',  'middle',
    ...
    ('spark_q11', v_topic_id, v_subtopic_id, 'mcq',  'senior',
    ...
    ('spark_q12', v_topic_id, v_subtopic_id, 'mcq',  'junior',
    ...
    ('spark_q13', v_topic_id, v_subtopic_id, 'mcq',  'middle',
    ...
    ('spark_q14', v_topic_id, v_subtopic_id, 'mcq',  'senior',
    ...
    ('spark_q15', v_topic_id, v_subtopic_id, 'mcq',  'leader',
    ...
    ('spark_q16', v_topic_id, v_subtopic_id, 'mcq',  'junior',
    ...
    ('spark_q17', v_topic_id, v_subtopic_id, 'mcq',  'middle',
    ...
    ('spark_q18', v_topic_id, v_subtopic_id, 'mcma', 'senior',
    ...
    ('spark_q19', v_topic_id, v_subtopic_id, 'mcq',  'leader',
    ...
    ('spark_q20', v_topic_id, v_subtopic_id, 'mcq',  'middle',
    ...
    ('spark_q21', v_topic_id, v_subtopic_id, 'mcq',  'junior',
    ...
    ('spark_q22', v_topic_id, v_subtopic_id, 'mcq',  'senior',
    ...
    ('spark_q23', v_topic_id, v_subtopic_id, 'mcq',  'junior',
    ...
    ('spark_q24', v_topic_id, v_subtopic_id, 'mcma', 'junior',
    ...
    ('spark_q25', v_topic_id, v_subtopic_id, 'mcq',  'middle',
  ...
  RAISE NOTICE 'Questions inserted (or already existed).';
  ...
  VALUES
    ...
    -- spark_q19
    ('spark_q19_a', 'spark_q19', 'Spark requests resource containers from the YARN Resource Manager, which allocates them based on cluster availability', true,  1),
    ('spark_q19_b', 'spark_q19', 'Spark bypasses YARN and directly communicates with individual worker nodes to claim resources', false, 2),
    ('spark_q19_c', 'spark_q19', 'YARN controls the Spark Driver and decides how many stages to create', false, 3),
    ('spark_q19_d', 'spark_q19', 'Spark and YARN share the same JVM process to reduce overhead in enterprise clusters', false, 4),

    -- spark_q20
    ('spark_q20_a', 'spark_q20', 'Kryo produces smaller, faster binary objects, reducing disk writes and network transfer during shuffles', true,  1),
    ('spark_q20_b', 'spark_q20', 'Kryo is the default serializer in Spark and requires no configuration', false, 2),
    ('spark_q20_c', 'spark_q20', 'Kryo encrypts data during serialization, making it more secure than Java serialization', false, 3),
    ('spark_q20_d', 'spark_q20', 'Kryo allows Spark to serialize Python objects natively without a JVM bridge', false, 4),

    -- spark_q21
    ('spark_q21_a', 'spark_q21', 'The entire job fails because the Driver process, which runs on the submitter machine, is lost', true,  1),
    ...
    ('spark_q21_d', 'spark_q21', 'Only the current stage fails; completed stages are preserved and the job continues', false, 4),

    -- spark_q22
    ('spark_q22_a', 'spark_q22', 'When you need fine-grained control over data partitioning or are processing unstructured data that does not fit a schema', true,  1),
    ('spark_q22_b', 'spark_q22', 'When performance is the top priority, because RDDs bypass the Catalyst Optimizer overhead', false, 2),
    ('spark_q22_c', 'spark_q22', 'When working with structured tabular data that has a well-defined schema', false, 3),
    ('spark_q22_d', 'spark_q22', 'When running SQL queries, because RDDs support SQL syntax natively', false, 4),

    -- spark_q23
    ('spark_q23_a', 'spark_q23', 'The Driver Program, which parses user code and builds the logical DAG before handing it to the DAG Scheduler', true,  1),
    ('spark_q23_b', 'spark_q23', 'The Executor, which converts bytecode into a DAG during task execution', false, 2),
    ('spark_q23_c', 'spark_q23', 'The Cluster Manager, which translates the application into a resource allocation graph', false, 3),
    ('spark_q23_d', 'spark_q23', 'The DAG Scheduler itself, which reads user code directly', false, 4),

    -- spark_q24 (mcma — multiple correct)
    ('spark_q24_a', 'spark_q24', 'Spark SQL',              true,  1),
    ('spark_q24_b', 'spark_q24', 'Structured Streaming',   true,  2),
    ('spark_q24_c', 'spark_q24', 'MLlib',                  true,  3),
    ('spark_q24_d', 'spark_q24', 'GraphX',                 true,  4),
    ('spark_q24_e', 'spark_q24', 'Apache Kafka',           false, 5),
    ('spark_q24_f', 'spark_q24', 'Apache Hadoop HDFS',     false, 6),

    -- spark_q25
    ('spark_q25_a', 'spark_q25', 'Spark keeps intermediate results in RAM between iterations, eliminating the disk I/O overhead that MapReduce incurs after every step', true,  1),
    ('spark_q25_b', 'spark_q25', 'Spark uses a more efficient sorting algorithm than MapReduce', false, 2),
    ('spark_q25_c', 'spark_q25', 'Spark runs on faster hardware because it requires SSDs while MapReduce uses HDDs', false, 3),
    ('spark_q25_d', 'spark_q25', 'Spark parallelizes more tasks per node than MapReduce due to a different threading model', false, 4)
  ON CONFLICT (option_id) DO NOTHING;

  RAISE NOTICE 'Answer options inserted (or already existed).';
